...
struct j_semantic_type : public j_base {
	...
	restrictive_ptr<jtl::String_Constant_Symbol>string_constant;
	restrictive_ptr<jtl::State_ID>				state_id;
	restrictive_ptr<jtl::State_Declaration_List>state_decl_list;
	restrictive_ptr<jtl::State_Specification_List>state_spec_list;
	restrictive_ptr<jtl::State_Declaration>		state_decl;
	restrictive_ptr<jtl::State_Specification>	state_spec;
	restrictive_ptr<jtl::Wrangler_Filter>		filter;
	restrictive_ptr<jtl::Filter_Specification>	filter_spec;
	restrictive_ptr<jtl::String_Constant_Symbol_List>string_list;
	restrictive_ptr<jtl::Type_Syntax_List>		type_syntax_list;
...
yy::Rule_Parsing_Unit::token_type
	yylex(yy::Rule_Parsing_Unit::semantic_type* yylval, yy::location* , jtl::Rule_Parser* i_parser);
...
public:
	...
	Rule_Implementor* parse(const std::string& irk_string);

	std::string get_lines(const yy::location& irk_loc);
	std::string get_line(unsigned i_line);
	~Rule_Parser();
private:
	Rule_Lexer* M_lexer;
};
