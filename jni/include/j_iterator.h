...
public:
	...
	typedef typename PairIter_t::value_type::second_type& reference;
	typedef typename PairIter_t::value_type::second_type* pointer;
...
public:
	...
	Const_PairIterator():M_iter(){}

	Const_PairIterator(PairIter_t i_iter):M_iter(i_iter){}

	Const_PairIterator& operator++(){ ++M_iter; return *this; }
	Const_PairIterator operator++(int){ Const_PairIterator temp(*this); ++M_iter; return temp; }
...
template<typename IterL, typename IterR>
bool operator!=(const Const_PairIterator<IterL>& irk_left, const Const_PairIterator<IterR>& irk_right){
	return !(irk_left.base() == irk_right.base());
}
...
template<typename Iter_t>
Const_PairIterator<Iter_t> make_pair_iter(Iter_t i_iter){
	return Const_PairIterator<Iter_t>(i_iter);
}
